...
public class QuotesService {
  ...
  private QuoteDto generateQuoteDto(
      String symbol, Company company, StockInfo stockInfo, Summary summary) {
    String ETF = "ETF";
    ...
    if (stockInfo != null) {
      InfoData infoData =
          stockInfo.getPrimaryData() != null
                  && !stockInfo.getPrimaryData().getLastTradeTimestamp().contains("AFTER HOURS")
              ? stockInfo.getPrimaryData()
              : stockInfo.getSecondaryData();
    ...
    if (summary != null && summary.getSummaryData() != null) {
      String avgVolStr =
          summary.getSummaryData().getAverageVolume() != null
              ? summary.getSummaryData().getAverageVolume().getValue()
              : summary.getSummaryData().getAvgDailyVol20Days().getValue();
