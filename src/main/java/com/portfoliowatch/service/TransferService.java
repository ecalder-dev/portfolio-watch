...
public class TransferService {
  ...
  public TransferDto createTransfer(TransferDto transferDto) throws NoDataException {
    ErrorHandler.validateNonNull(
        transferDto.getSymbol(), "TransferDto's symbol should not be null.");
    ErrorHandler.validateNonNull(
        transferDto.getShares(), "TransferDto's shares should not be null.");
    ErrorHandler.validateNonNull(
        transferDto.getDateTransacted(), "TransferDto's dateTransacted should not be null.");
    ErrorHandler.validateNonNull(
        transferDto.getFromAccount(), "TransferDto's oldAccount should not be null.");
    ErrorHandler.validateNonNull(
        transferDto.getToAccount(), "TransferDto's newAccount should not be null.");

    Account oldAccount =
        accountRepository
            .findById(transferDto.getFromAccount().getId())
            .orElseThrow(
                () ->
                    new NoDataException(
                        "Account not found with id " + transferDto.getFromAccount().getId()));
    Account newAccount =
        accountRepository
            .findById(transferDto.getToAccount().getId())
            .orElseThrow(
                () ->
                    new NoDataException(
                        "Account not found with id " + transferDto.getToAccount().getId()));
    BigDecimal currentTotalShares = lotService.getTotalShares(oldAccount, transferDto.getSymbol());
    ErrorHandler.validateTrue(
        currentTotalShares.compareTo(transferDto.getShares()) >= 0,
        String.format(
            "There are not enough shares to process transfer. Requested shares: %f, Actual shares: %f.",
            transferDto.getShares(), currentTotalShares));

    Transfer savedTransfer =
        transferRepository.save(transferDto.generateTransfer(oldAccount, newAccount));
